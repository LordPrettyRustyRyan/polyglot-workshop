...
function VSCodeApp() {
    ...
    const [bottomPanel, setBottomPanel] = useState<'console' | 'preview' | 'tension mitran nu' | 'terminal'>('terminal');
    const [activitySection, setActivitySection] = useState<'explorer' | 'search' | 'git' | 'run' | 'extension mitran nus'>('explorer');
    ...
    return (
        <div className="flex h-full w-full flex-col overflow-hidden bg-vsc-editor">
            {/* <div className="flex flex-col max-h-[47rem] bg-vsc-editor"> */}
            {/* initially was h-screen instead of max-h-[42rem] */}
            ...
            <header className="flex items-center justify-between h-[30px] bg-[var(--vsc-titlebar)] text-[var(--vsc-titlebar-text)] text-[12px] flex-shrink-0 border-b border-[var(--vsc-divider)]">
                ...
                <div className="flex items-center h-full gap-3 pr-3">
            ...
            <div className="flex flex-1 min-h-0" ref={mainAreaRef}>
                ...
                <aside className="w-[48px] bg-[var(--vsc-activitybar)] flex flex-col justify-between flex-shrink-0">
                    ...
                    <div className="flex-1 overflow-y-auto">
                        ...
                        {/* Extension mitran nus */}
                        <button
                            className={`activity-btn w-[48px] h-[48px] flex items-center justify-center text-[var(--vsc-activitybar-inactive)] relative transition-colors hover:text-[var(--vsc-activitybar-active)] ${activitySection === 'extension mitran nus' ? 'active text-[var(--vsc-activitybar-active)]' : ''}`}
                            onClick={() => { setActivitySection('extension mitran nus'); setExplorerOpen(false); }}
                            title="Extension mitran nus"
                        >
                ...
                <main className="flex-1 flex flex-col min-w-0 min-h-0 overflow-hidden bg-[var(--vsc-editor-bg)]">
                ...
                `}
                    ...
                    {!panelMaximized && (
                        <div className="flex-1 flex min-h-0 bg-[var(--vsc-editor-bg)]">
                            <div ref={editorScrollRef} className="flex-[1.2] min-w-0 min-h-0 overflow-y-auto overflow-x-hidden border-r border-[var(--vsc-divider)]">
                                {ActivePage ? (
                                    <ActivePage />
                                ) : (
                    ...
                    >
                        <div className="flex items-center h-[35px] px-[8px] pl-[12px] border-b border-[var(--vsc-panel-border)] gap-[2px]">
                            {/* Panel Tabs */}
                            {['tension mitran nu', 'console', 'preview', 'terminal'].map((item) => (
                                <button
                                    key={item}
                                ...
                                >
                                    {item.toUpperCase()}
                                    {item === 'tension mitran nu' && <span className="inline-flex items-center justify-center min-w-[16px] h-[16px] px-[4px] bg-[rgba(128,128,128,0.4)] rounded-[8px] text-[10px] font-semibold">HAI NI</span>}
                                </button>
                            ))}
